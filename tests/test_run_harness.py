...
from ohdyn.analytic_delayed_map_null_gate import (
    NULL_GATE_CONDITIONS,
    NULL_GATE_FIELDS,
    NULL_GATE_SCIENTIFIC_STATUS,
    NULL_GATE_STATUS,
...
def test_analytic_delayed_map_null_gate_emits_preregistered_conditions(
    tmp_path: Path,
) -> None:
    rows_a = run_analytic_delayed_map_null_gate(
        config_path=ANALYTIC_DELAYED_MAP_NULL_GATE,
        out_dir=tmp_path / "null_gate_a",
    )
    rows_b = run_analytic_delayed_map_null_gate(
        config_path=ANALYTIC_DELAYED_MAP_NULL_GATE,
        out_dir=tmp_path / "null_gate_b",
    )

    assert rows_a == rows_b
    assert (
        tmp_path / "null_gate_a" / "null_gate_summary.csv"
    ).read_bytes() == (
        tmp_path / "null_gate_b" / "null_gate_summary.csv"
    ).read_bytes()
    assert [row["condition_id"] for row in rows_a] == list(NULL_GATE_CONDITIONS)
    assert len(rows_a) == 4
    assert {row["diagnostic_status"] for row in rows_a} == {NULL_GATE_STATUS}
...
def test_analytic_delayed_map_null_gate_writes_summary_only_artifacts(
    tmp_path: Path,
) -> None:
    out_dir = tmp_path / "analytic_delayed_map_null_gate"

    rows = run_analytic_delayed_map_null_gate(
        config_path=ANALYTIC_DELAYED_MAP_NULL_GATE,
        out_dir=out_dir,
    )
    ...
    with (out_dir / "null_gate_summary.csv").open() as handle:
        assert next(csv.reader(handle)) == list(NULL_GATE_FIELDS)
        csv_rows = list(csv.DictReader(handle, fieldnames=NULL_GATE_FIELDS))
    assert len(csv_rows) == 4
    manifest = yaml.safe_load((out_dir / "manifest.yaml").read_text())
    assert manifest["status"] == NULL_GATE_STATUS
    assert manifest["scientific_status"] == NULL_GATE_SCIENTIFIC_STATUS
    assert manifest["conditions"] == list(NULL_GATE_CONDITIONS)
...
def test_automation_guard_closes_for_current_a5_post_smoke_result_wording(
    tmp_path: Path,
) -> None:
    status_path = tmp_path / "AUTOMATION_STATUS.md"
    review_path = tmp_path / "latest-review.md"
    ...
    a5_path.write_text("# A5 Single-Hive Anticipatory Predictive-Control Preregistration\n")
    action = (
        "review the current bounded A5 seed `5,6` smoke/analyzer result "
    ...
    status_path.write_text(
        "\n".join(
            [
                ...
                "## Current Focus",
                "",
                "Source-of-truth status: Ben's current automation prompt "
                "explicitly reauthorizes the bounded A5 single-hive "
                "anticipatory predictive-control stage.",
                "Current A5 interpretation boundary: the current bounded seed "
                "`5,6` smoke/analyzer rerun improved forecast skill for "
                "intermediate predictors, but no intermediate predictor "
                "passed all residual/null, oracle-nontriviality, compression, "
                "and guardrail promotion criteria. A5 remains fail-closed.",
                "",
                "## Recommended Next Step",
                "",
                f"- Recommended next step: {action}",
...
def test_automation_guard_closes_current_a5_reopening_under_a5_a7_3_recovery_review(
    tmp_path: Path,
) -> None:
    status_path = tmp_path / "AUTOMATION_STATUS.md"
    review_path = tmp_path / "latest-review.md"
    ...
    a5_path.write_text("# A5 Single-Hive Anticipatory Predictive-Control Preregistration\n")
    action = (
        "review the existing bounded A5 seed `5,6` smoke/analyzer result "
        "before authorizing any larger A5 holdout."
    )
    review_action = (
    ...
    status_path.write_text(
        "\n".join(
            [
                ...
                "## Current Focus",
                "",
                "Source-of-truth status: the current recurring automation prompt "
                "explicitly selects A5 single-hive anticipatory predictive-control "
                "dynamics as the bounded scientific stage.",
                "Current active task: preserve the concise A5 preregistration, "
                "keep the existing deterministic single-hive scaffold as the only "
                "authorized smoke/pilot surface, and avoid broad new mechanics.",
                "Current interpretation boundary: the repeated bounded seed `5,6` "
...
def test_automation_guard_opens_for_ben_selected_analytic_delayed_map_pivot(
    tmp_path: Path,
) -> None:
    status_path = tmp_path / "AUTOMATION_STATUS.md"
    review_path = tmp_path / "latest-review.md"
    ...
    a5_path.write_text("# A5 Single-Hive Anticipatory Predictive-Control Preregistration\n")
    action = (
        "implement the smallest analytic delayed resource-bounded prediction "
        "map over rho, delta, mu, kappa, and nu before adding simulator "
        "mechanics."
    )
    review_action = (
        "Recover governance by closing the reopened bounded A5 pass as "
    ...
    status_path.write_text(
        "\n".join(
            [
                ...
                "## Current Focus",
                "",
                "Source-of-truth status: Ben's 2026-06-30 instruction selects "
                "the analytic delayed-map pivot as the active next OmegaSim "
                "gate, not A5.2.",
                "This does not reopen A5.2 or authorize broad A5 simulator "
                "work. Prior A5 evidence remains fail-closed background.",
                "",
                "## Recommended Next Step",
                "",
                f"- Recommended next step: {action}",
    ...
    status_path.write_text(
        "\n".join(
            [
                ...
                "and remains the active preregistration summary for this "
                "bounded smoke/pilot.",
                "Ben's 2026-06-29 automation instruction explicitly sets A5 "
                "single-hive anticipatory predictive-control dynamics as the "
                "bounded current scientific stage.",
                "",
                "## Recommended Next Step",
                "",
                f"- Recommended next step: {a5_action}",
...
def test_automation_guard_current_a5_ask_ben_next_axis_closes_over_major_review(
    tmp_path: Path,
) -> None:
    status_path = tmp_path / "AUTOMATION_STATUS.md"
    review_path = tmp_path / "latest-review.md"
    ...
    a5_path.write_text("# A5 Single-Hive Anticipatory Predictive-Control Preregistration\n")
    a5_action = (
        "ask Ben for the next preregistered scientific axis; until then, keep "
        "OmegaSim automation closed/no-op with A5 fail-closed at the seed "
        "`5,6` smoke/analyzer boundary."
    )
    review_action = (
        "Quarantine the uncommitted A7.3 residual-analyzer WIP and patch the "
        "guard/status so the current A5 fail-closed fresh preregistration "
        "required state is closed/no-op pending Ben's choice."
    )
    status_path.write_text(
        "\n".join(
            [
                ...
                "",
                "Source-of-truth status: the current automation prompt "
                "explicitly requests A5 anticipatory predictive-control "
                "dynamics as the bounded next scientific stage.",
                "Current concise A5 gate: explicit single-hive A5 reopening "
                "and active preregistration summary are recorded in "
                "`docs/a5_single_hive_anticipatory_predictive_control_preregistration.md`.",
                "Current active task: preserve the concise A5 preregistration "
                "and existing minimal deterministic smoke scaffold.",
                "",
                "## Recommended Next Step",
                "",
                f"- Recommended next step: {a5_action}",
    ...
    assert state["repo_write_allowed"] is False
    assert state["closed_reasons"] == [
        "automation_status_a5_awaiting_next_axis"
    ]
    assert state["strategic_change_level"] == "major"
    assert state["notify_ben"] is True
    assert state["recommended_next_action"] == a5_action
